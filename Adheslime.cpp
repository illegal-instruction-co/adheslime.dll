...
class V8ScriptEngine final : public ac::Component {
public:
    typedef void (*NativeHandler)(void* context);
    inline static std::vector<std::pair<uint64_t, NativeHandler>> nativeHandlers;

    const char* GetName() const override { return X("ac::ScriptingComponent").c_str(); }
...
__declspec(allocate(".adhdata")) static pDetectionFunc detectionDispatch[] = {
    ...
    []() { 
        if (auto v8 = (V8ScriptEngine*)ac::ComponentRegistry::GetInstance().GetComponent("ac::ScriptingComponent")) v8->ExecuteDetectionRules();
    }
};
...
public:
    void Initialize() {
        ...
        ac::ComponentRegistry::GetInstance().Register(v8);
...
// --- Exports ---
extern "C" __declspec(dllexport) ac::Component* CreateComponent() {
    if (!clientInstance) {
        clientInstance = new AdheslimeClient();
        clientInstance->Initialize();
    }
    return (ac::Component*)ac::ComponentRegistry::GetInstance().GetComponent("ac::ScriptingComponent");
}
